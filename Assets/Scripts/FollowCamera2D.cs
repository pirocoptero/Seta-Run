...
namespace CustomCamera
{
    ...
    public class FollowCamera2D : MonoBehaviour
    {
        ...
        void LateUpdate ()
        {
            Vector3 screenPoint = camera.WorldToViewportPoint(diamond.position);
            bool onScreen = screenPoint.z > 0 && screenPoint.x > 0 && screenPoint.x < 1 && screenPoint.y > 0 && screenPoint.y < 1;

            if(onScreen){
                find = true;
                Vector3 smoothedPosition = Vector3.Lerp(transform.position, new Vector3(diamond.position.x, transform.position.y, transform.position.z), smoothSpeed);
